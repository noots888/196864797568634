var App={
  version:'mymap-v3-1-236_reset_ui_detail',
  versionShort:'myMap v3.1.236' ,
...
/* myMap v3.1.200: handle re-select resume imports. */
(function(){
  if(!window.UI||UI.__v200ResumeHandleFiles)return; UI.__v200ResumeHandleFiles=true;
  const oldHandle=UI.handleFiles?.bind(UI);
  UI.handleFiles=async function(files){
    const resume=!!window.myMapResumeImportPending;
    window.myMapResumeImportPending=false;
    const list=Array.from(files||[]).filter(Boolean);
    if(!list.length)return;
    try{
      this.progress(true,resume?'Resuming import…':'Importing files…',`${list.length} file(s) selected`,2);
      const res=await ImportEngine.importFiles(list,{resumeSelectedQueue:resume});
      if(res?.needsFullRebuild){await (SearchEngine.rebuildAsync?SearchEngine.rebuildAsync('Rebuilding data index'):SearchEngine.rebuild());}
      this.refreshAll(); window.LeanMapApp?.renderCircuitList?.(); window.LeanMapApp?.closeCircuitPicker?.();
      const skipped=Number(res?.skippedUnchanged||0); const imported=Number(res?.imported||0);
      if(resume)this.toast(`Resume complete: ${imported.toLocaleString()} assets imported${skipped?` · ${skipped} file(s) already done`:''}.`);
      else if(skipped&&skipped>=list.length&&!imported)this.toast('No changed files found. Existing indexed data kept.');
      else this.toast(`Imported ${imported.toLocaleString()} assets from ${(res.files||[]).length||list.length} file(s)${skipped?` · ${skipped} unchanged skipped`:''}.`);
    }catch(err){Diagnostics.capture(err); this.toast(err?.name==='AbortError'?'Import stopped. Re-select same files to resume.':'Import failed. Re-select same files to continue from completed files.');}
    finally{this.progress(false); const input=document.getElementById('fileInput'); if(input)input.value='';}
  };
})();
